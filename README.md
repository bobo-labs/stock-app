...
| `POINT_PILOT_RECEIPT_ENABLED` | Set to `true` to automatically print the non-tax pilot receipt after an approved Point card sale. Disabled by default. |
...
| `POINT_PILOT_RECEIPT_LOGO_PATH` | Optional server-side PNG/JPEG logo path. Railway uses the bundled monochrome Atelier logo when omitted. |
...
The optional pilot receipt is a demonstration print generated from Bakery POS's immutable sale lines after Mercado Pago confirms a card payment. It is sent through the Point Printing API as a monochrome PNG and is explicitly labelled **not tax-valid**. It does not create, replace, or simulate an SII DTE, CAF, folio, TED, or electronic signature. Keep `POINT_PILOT_RECEIPT_ENABLED` unset in normal production until the pilot is intentionally enabled.

Printing is idempotent per sale. A webhook and browser poll can confirm the same payment concurrently without creating duplicate prints. Bakery POS records `sent` when Mercado Pago accepts the action and then checks the action until Point reports `processed`; failed prints are recorded on the sale and can be retried from its details. The payment order keeps `print_on_terminal: no_ticket`, so enabling the pilot prints one customized business copy instead of both the standard seller ticket and the pilot receipt.
